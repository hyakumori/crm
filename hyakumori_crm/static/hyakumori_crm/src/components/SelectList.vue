...
export default {
  ...
  methods: {
    reiszeInputPlaceHolderWidth() {
      ...
      const input = selectAll(this.$refs.selectList)._groups[0].$refs.input;
      const placeHolderLength = input.placeholder.length;
      input.size =
        placeHolderLength > inputMinWidth ? placeHolderLength : inputMinWidth;
    },
